...
Here I will describe and analyze Snow Leopard detections from camera traps set in Kirghiztan from 2015-2019.

# 1 Data Import and formating 

I used the excel file named "Données_PP_Naryn_2016_2019" 
...
Here we import the necessary data, camera trap detections fill and Kyrgyzstan shapefile. 


Notice that I made some few change directly on the raw data. otherwise most of the formating is done directly in the script  
I reverse Longitude and Latitude in the excel sheet as the  label seemed to have been reversed (Only after I changed the columns names, that trap locations fell into Kirgizstan shpefile). 
...
#Becarefull between 250 and 2019 many Longitude has two comas, might woth checking the GPS coordinate
...
#Here for the pictures collected in 2019, I added 4 years and 43 days to any date recorded as 2015
...
Here we show the yearly trajectoy taken by each individual across traps. Unfortunatly we can not delimitate a Minimum convex Polygone for individuals' territory as we only have fixed trap locations no GPS data. Probably something could be done if we combine with the genetic samples.  
```{r message=FALSE, warning=FALSE}
